...
namespace TEAM
{
    public partial class Form_PIT : FormBase
    {
        //private readonly FormMain _myParent;
        public Form_PIT(FormMain parent)
        {
            MyParent = parent;
            InitializeComponent();
        ...
        private void butGenerate_Click(object sender, EventArgs e)
        {
            ...
            foreach (var tableName in tableList.Keys)
            {
                ...
                foreach (var columnName in columnNames)
                {
                    if (checkBoxInheritZeroSatelliteKey.Checked)
                    {
                        ...
                        {
                            outputQuery.AppendLine("  COALESCE([" + tableName + "." + columnName + "]," + "" + ") AS [" + tableName + "." + columnName + "],");
                        }
                        else
                        {
            ...
            outputQuery.AppendLine("  SELECT");
            outputQuery.AppendLine("    *,");
            outputQuery.AppendLine("    LAG(ATTRIBUTE_CHECKSUM, 1, " + ""+ ") OVER(PARTITION BY [" + hubTable + "." + hubKey + "] ORDER BY PIT_EFFECTIVE_DATETIME ASC) AS PREVIOUS_ATTRIBUTE_CHECKSUM");
            outputQuery.AppendLine("  FROM");
            outputQuery.AppendLine("  (");
            outputQuery.AppendLine("    SELECT *,");
